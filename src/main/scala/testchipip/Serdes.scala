...
class TLMergedBundle(params: TLBundleParameters, hasCorruptDenied: Boolean = true) extends TLBundleBase(params) {
  ...
  val opcode = UInt(3.W)
  val param = UInt(
    Seq(TLAtomics.width, TLHints.width, TLPermissions.aWidth, TLPermissions.bdWidth, TLPermissions.cWidth).max.W
  )
...
object TLMergedBundle {
  ...
  def apply(
    chan:             DecoupledIO[TLChannel],
    hasCorruptDenied: Boolean
  )(
    implicit edge: TLEdge
  ): DecoupledIO[TLMergedBundle] =
    apply(chan, chan.bits.params, hasCorruptDenied)

  def apply(
    chan:             DecoupledIO[TLChannel],
    params:           TLBundleParameters,
    hasCorruptDenied: Boolean
  )(
    implicit edge: TLEdge
  ): DecoupledIO[TLMergedBundle] = {
  ...
  def toA(
    chan:             DecoupledIO[TLMergedBundle],
    params:           TLBundleParameters,
    hasCorruptDenied: Boolean
  ): DecoupledIO[TLBundleA] = {
...
class TLSerdes(
  ...
  val node = TLManagerNode(
    params.map(manager =>
      TLSlavePortParameters.v1(managers = Seq(manager), beatBytes = beatBytes, endSinkId = endSinkId)
    )
  )
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    node.in.zip(io.ser).zipWithIndex.foreach {
      case (((tl, edge), ser), i) =>
        ...
        val outArb      = Module(new HellaPeekingArbiter(mergeType, outChannels.size, (b: TLMergedBundle) => b.last))
        ...
        inDes.io.out.ready := MuxLookup(
          inDes.io.out.bits.chanId,
          false.B,
          Seq(TLMergedBundle.TL_CHAN_ID_B -> tl.b.ready, TLMergedBundle.TL_CHAN_ID_D -> tl.d.ready)
        )
...
class TLDesser(w: Int, params: Seq[TLClientParameters], hasCorruptDenied: Boolean = true)(implicit p: Parameters)
    extends LazyModule {

  val node = TLClientNode(params.map(client => TLMasterPortParameters.v1(Seq(client))))
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    node.out.zip(io.ser).zipWithIndex.foreach {
      case (((tl, edge), ser), i) =>
        val mergeType = new TLMergedBundle(tl.params, hasCorruptDenied)
...
class TLSerdesser(
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val outChannels = Seq(manager_tl.e, client_tl.d, manager_tl.c, client_tl.b, manager_tl.a)
    val outArb      = Module(new HellaPeekingArbiter(mergeType, outChannels.size, (b: TLMergedBundle) => b.last))
    val outSer      = Module(new GenericSerializer(mergeType, w))
