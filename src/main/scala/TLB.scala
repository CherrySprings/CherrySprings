...
class TLB(implicit p: Parameters) extends CherrySpringsModule with Sv39Parameters {
  ...
  // TLB - 4 KB page
  val array4kb       = Module(new SRAM(16, tlb4kbEntryLen))
  val array4kb_valid = RegInit(VecInit(Seq.fill(16)(false.B))) // not "valid" in PTE
  ...
  array4kb.io.wen     := io.ptw.resp.fire && (pt_level === 0.U)
  ...
  switch(state) {
    ...
    is(s_tlb) {
      state    := Mux(hit4kb, s_resp, s_ptw_req)
      pt_level := Mux(hit4kb, 0.U, (pageTableLevels - 1).U)
    }
    is(s_ptw_req) {
