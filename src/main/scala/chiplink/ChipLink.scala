...
class ChipLink(val params: ChipLinkParams)(implicit p: Parameters) extends LazyModule() {
  ...
  private val slaveNode = TLManagerNode(
    Seq(
  ...
  private val masterNode = TLClientNode(
    Seq(
  ...
  lazy val mmap = {
    ...
    params.copy(TLUH = AddressSet.unify(tluh.flatMap(_.address)), TLC = AddressSet.unify(tlc.flatMap(_.address)))
  }
  ...
  lazy val module = new LazyModuleImp(this) {
    ...
    require(
      edgeIn.client.clients.filter(_.supports.probe).size <= 1,
