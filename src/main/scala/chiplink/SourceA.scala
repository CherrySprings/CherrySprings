...
class SourceA(info: ChipLinkInfo) extends Module {
  ...
  private def writeable(m: TLManagerParameters): Boolean =
    if (m.supportsAcquireB) m.supportsAcquireT else m.supportsPutFull
  ...
  (cams.zip(a_sel.asBools)).foreach {
    case (cam, sel) =>
  ...
  (cams.zip(d_sel.asBools)).foreach {
    case (cam, sel) =>
