...
class StuckSnooperNode(uFn: Seq[TLClientPortParameters] => TLClientPortParameters)(implicit valName: ValName)
  ...
  def resolveStar(iKnown: Int, oKnown: Int, iStars: Int, oStars: Int): (Int, Int) = {
    ...
    require(oKnown == 1, s"StuckSnooper node expects exactly one output ${iKnown} ${oKnown} ${iStars} ${oStars}")
    (0, 0)
  }
...
class StuckSnooper(uFn: Seq[TLClientPortParameters] => TLClientPortParameters)(implicit p: Parameters)
    extends LazyModule {
