...
class UART(implicit p: Parameters) extends LazyModule with HasCherrySpringsParameters {
  ...
  lazy val module = new UARTModule(this)
}

class UARTModule(outer: UART) extends LazyModuleImp(outer) with HasCherrySpringsParameters {
  val RxFifoDepth = 4
  val TxFifoDepth = 4
  ...
  val (tl, edge) = outer.node.in.head
